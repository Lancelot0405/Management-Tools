# Management Tools — AGENTS.md (Tối ưu cho Antigravity)

## Quy tắc bắt buộc dành cho Antigravity
...
- **Quy trình Lập kế hoạch (Planning Mode)**: 
  - Trước khi triển khai thay đổi lớn, Antigravity phải tạo/cập nhật `implementation_plan.md` ở thư mục artifacts và đặt `request_feedback = true` để xin ý kiến từ USER.
  ...
  - Khi bắt đầu thực hiện, tạo `task.md` làm danh sách việc cần làm (TODO list).
  - Khi hoàn thành, tạo `walkthrough.md` tổng hợp các thay đổi và kết quả xác minh.
- **Tra cứu tài liệu cục bộ**: Đối với bất kỳ component UI nào của HeroUI v3, hãy luôn dùng `view_file` để tra cứu tài liệu tương ứng tại thư mục `.heroui-docs/react/components/<component-name>.mdx` trước khi viết mã nguồn.
- **Build & Test sạch**: Luôn chạy `npm run build` để kiểm tra kiểu và build thành công, đồng thời chạy `npm run test` để chạy các unit test trước khi hoàn tất nhiệm vụ.
- **Không thêm tính năng ngoài yêu cầu**: Tránh thêm các tính năng phát sinh không có trong mô tả yêu cầu.

---

## Tổng quan dự án

Management Tools là một **Progressive Web App (PWA)** quản lý hoạt động F&B (Food & Beverage) cho các sự kiện và lễ hội. Ứng dụng được tối ưu cho mobile với hỗ trợ offline và thông báo push.
...
- **Production branch của project KHÔNG phải `final-gemini`.** Mỗi lần push lên
  ...
  domain production `management-tools.vercel.app`.
